"""HuggingFace model manager for Mac agent.
...
# ── Curated model catalog for Apple Silicon Macs ─────────────────────────────
# (hf_repo, gguf_filename_pattern, display_name, size_gb, params_b, max_ctx)
# Covers Macs from 8GB (M1 Air) to 192GB (M4 Ultra).
_BUILTIN = [
    # Small models — fit on 8-16GB Macs
    ...
    # XL models — 64GB+ Macs (M2/M3/M4 Max/Ultra)
...
def _search_hf_gguf(query="GGUF", limit=50):
    ...
    try:
        ...
        req.add_header("User-Agent", "ClusterFlock-Mac/0.1")
...
def _detect_quant(filename):
    ...
    for q in ["q4_k_m", "q4_k_s", "q4_0", "q5_k_m", "q5_k_s", "q8_0",
              "q6_k", "q3_k_m", "q2_k", "iq4_nl", "fp16", "f16"]:
        if q in lower:
            return q
    return "unknown"
...
# ── Model Selection (bin-packing for unified memory) ─────────────────────────

def _memory_estimate_mb(size_gb, params_b, ctx_length):
    """Estimate total memory needed: model weights + KV cache.

    With f16 KV cache (Metal default), uses full precision.
    """
    weights_mb = size_gb * 1024
    # KV cache: params_B × bytes_per_token × context
    # f16 cache: ~2400 bytes per token per billion params
    kv_cache_mb = (params_b * 2400 * ctx_length) / (1024 * 1024)
    return weights_mb + kv_cache_mb


def pick_best_models(gpus, catalog, tight_pack=False):
    """Greedy bin-packing: select models that fit in available unified memory.

    Apple Silicon always has unified memory — one big pool shared with the OS.
    We leave headroom for macOS and other apps.
    """
    ...
    # Safety margin: 75% of free memory (macOS needs more headroom than Linux)
    budget_mb = total_free_mb * 0.75
    assignments = []
    used_mb = 0
