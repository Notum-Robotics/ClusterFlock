"""Hardware profiling for DGX Spark.

Optimized for GB10 with 128GB unified LPDDR5X memory.
    ...
    try:
        ...
        for line in out.strip().splitlines():
            ...
            if len(p) >= 4:
                ...
                mem_free = p[2]
                util = p[3]
                # DGX Spark (GB10) reports [N/A] for memory — unified memory
...
def is_dgx_spark():
    ...
    for g in gpus:
        ...
        if "gb10" in name or "dgx" in name or "blackwell" in name:
            return True
    # Also check via nvidia-smi directly for compute capability
