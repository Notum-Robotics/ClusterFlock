"""Command dispatcher for Spark agent.
...
def execute(cmd):
    ...
    if action == "unload_all":
        ...
        if _current_model and get_bench(_current_model) == 0:
            _set_activity("benchmarking", _current_model)
            try:
                perf = benchmark()
        ...
        _set_activity("generating", _current_model)
        try:
...
def _detect_running_model():
    ...
    if models:
        _current_model = models[0].get("id", "")
        print(f"[detect] Found running model: {_current_model}")
...
def _load_model(model_id, context_length=None, model_path=None):
    ...
    print(f"[load] Loading {model_id} (ctx={ctx})...")
...
def _download_and_load(model_id, context_length=None):
    ...
    # Parse model_id: could be "Repo/Name/quant" or just "Repo/Name"
