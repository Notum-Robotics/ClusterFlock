"""HuggingFace model manager — unified for all platforms.
...
# Back-compat: older runtime code imports this symbol to detect TensorRT NVFP4
# engine directories. In the GGUF-only path this remains empty.
def local_nvfp4_engines():
    return []
...
_BUILTIN = [
    # Qwen3.5 series
    ("Qwen/Qwen3.5-27B-GGUF",         "q4_k_m",  "Qwen3.5 27B",          16.2,   27, 131072),
    ("Qwen/Qwen3.5-9B-GGUF",          "q4_k_m",  "Qwen3.5 9B",            5.4,    9, 131072),
    ("Qwen/Qwen3.5-4B-GGUF",          "q4_k_m",  "Qwen3.5 4B",            2.3,    4, 131072),
    # Qwen3 series
    ...
    # Qwen3 Coder
    ("Qwen/Qwen3-Coder-30B-A3B-GGUF", "q4_k_m",  "Qwen3 Coder 30B",      17.4,   30, 262144),
    # DeepSeek R1 distills
    ("deepseek-ai/DeepSeek-R1-Distill-Qwen-32B-GGUF", "q4_k_m", "DeepSeek R1 32B", 19.9, 32, 131072),
    ("deepseek-ai/DeepSeek-R1-Distill-Qwen-14B-GGUF", "q4_k_m", "DeepSeek R1 14B",  8.5, 14, 131072),
    # Gemma 3
    ("ggml-org/gemma-3-27b-it-GGUF",  "q4_k_m",  "Gemma 3 27B",          17.2,   27, 131072),
    ("ggml-org/gemma-3-12b-it-GGUF",  "q4_k_m",  "Gemma 3 12B",           7.3,   12, 131072),
    ("ggml-org/gemma-3-4b-it-GGUF",   "q4_k_m",  "Gemma 3 4B",            3.0,    4, 131072),
    # Llama 3.3
    ("meta-llama/Llama-3.3-70B-Instruct-GGUF", "q4_k_m", "Llama 3.3 70B", 42.5,  70, 131072),
    # Mistral
    ("mistralai/Mistral-Small-3.2-24B-Instruct-2503-GGUF", "q4_k_m", "Mistral Small 24B", 15.0, 24, 131072),
    # Phi-4
...
def get_bench(model_id, device=None):
    """Return cached tokens_per_sec for a model on a specific device, or 0.

    Uses device-qualified key (model_id:device) when device is given.
    """
    if device:
        key = f"{model_id}:{device}"
        return _bench_cache.get(key, {}).get("tokens_per_sec", 0)
    return _bench_cache.get(model_id, {}).get("tokens_per_sec", 0)


def save_bench(model_id, perf, device=None):
    """Save benchmark result for a model, optionally qualified by device."""
    key = f"{model_id}:{device}" if device else model_id
    _bench_cache[key] = {**perf, "timestamp": time.time()}
    _save_bench_cache()
...
def local_models():
    ...
    for f in sorted(MODELS_DIR.rglob("*.gguf")):
        ...
        group = shard_groups.setdefault(base_id, {
            "id": base_id,
            "path": None,
            "size_bytes": 0,
            "first_idx": None,
        ...
        group["size_bytes"] += f.stat().st_size
        idx = int(m.group("idx"))
        if group["first_idx"] is None or idx < group["first_idx"]:
            group["first_idx"] = idx
            group["path"] = str(f)

    for gid, g in sorted(shard_groups.items()):
        base_name = Path(gid).name[:-5]  # strip trailing ".gguf"
        name = base_name.replace("-", " ").replace("_", " ").title()
        models.append({
            "id": gid,
            "name": name,
            "path": g["path"],
            "size_gb": round(g["size_bytes"] / (1024**3), 1),
            "downloaded": True,
        })
...
def _search_hf_gguf(query="GGUF", limit=50):
    ...
    try:
        ...
        req = _hf_request(f"{HF_API}{params}")
...
def _pick_mmproj_file(files):
    candidates = [f for f in files if f.lower().endswith(".gguf") and "mmproj" in f.lower()]
    if not candidates:
        return None
    ...
    def rank(path):
        ...
        return (3, len(path), name)

    return sorted(candidates, key=rank)[0]


def _download_mmproj(hf_repo, dest_dir):
    """Download a multimodal projector GGUF if the repo provides one."""
    try:
        mmproj_name = None
        hf_token = os.environ.get("HF_TOKEN")
        ...
        if _HAS_HF_HUB:
            ...
            mmproj_name = _pick_mmproj_file(files)
            if not mmproj_name:
                return
            ...
            log.info(f"[download] Fetching vision projector '{mmproj_name}'...")
            hf_hub_download(
                repo_id=hf_repo,
            ...
            log.info(f"[download] ✓ Vision projector: {dest_path}")
            return
        ...
        url = f"https://huggingface.co/{hf_repo}/resolve/main/{mmproj_name}"
        curl_cmd = ["curl", "-L", "-o", str(dest_path), url]
        if hf_token:
            curl_cmd[1:1] = ["-H", f"Authorization: Bearer {hf_token}"]

        log.info(f"[download] Fetching vision projector '{mmproj_name}'...")
        r = subprocess.run(curl_cmd, timeout=1800)
        if r.returncode != 0:
...
def download_model(hf_repo, quant="q4_k_m", filename=None):
    ...
    if filename:
        ...
        # Download exact file — skip quant pattern logic below
        _download_state.update(active=True, model=hf_repo.split("/")[-1],
                               expected_bytes=0, dest_dir=str(dest_dir))
        try:
            if _HAS_HF_HUB:
    ...
    if existing:
        ...
        for f in existing:
            if "00001-of-" in f.name:
                log.info(f"[download] Already exists: {f}")
                return str(f)
        log.info(f"[download] Already exists: {existing[0]}")
...
def _download_via_hub(hf_repo, quant, filename, dest_dir):
    ...
    if first_shards:
        ...
        log.info(f"[download] ✓ {result.name} ({size_gb} GB across {len(gguf_files)} shards)")
        return str(result)
...
def _memory_estimate_mb(size_gb, params_b, ctx_length, kv_cache_type="q4_0"):
    """Estimate total memory needed: model weights + KV cache."""
    weights_mb = size_gb * 1024
    # f16 cache: ~2400 bytes/tok/B, q4_0 cache: ~600 bytes/tok/B
    bpt = 600 if kv_cache_type in ("q4_0", "q4_1") else 2400
    kv_cache_mb = (params_b * bpt * ctx_length) / (1024 * 1024)
