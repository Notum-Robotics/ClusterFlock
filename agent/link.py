...
def _run_cmd(cmd, command_fn, address, token):
    ...
    # Deliver result to orchestrator with retries
    url = f"{address}/api/v1/results/{task_id}"
    last_err = None
    for attempt in range(3):
        try:
            _post(url, result, token, timeout=min(cmd.get("ttl", 120), 300))
            return  # success
        except Exception as e:
            last_err = e
            if attempt < 2:
    ...
    try:
        ...
        pass  # truly unreachable orchestrator
